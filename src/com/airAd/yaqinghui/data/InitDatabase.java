...
public class InitDatabase extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE cep_events (cid INTEGER PRIMARY KEY  AUTOINCREMENT  NOT NULL , "
				+ "cep_id VARCHAR(30), event_id VARCHAR(30), "
				+ "appointment_flag INTEGER, checkin_flag INTEGER, "
				+ "mark_flag INTEGER, user_id VARCHAR(30), add_time LONG);");
		// 勋章记录表
		db.execSQL("CREATE TABLE medals (cid INTEGER PRIMARY KEY  AUTOINCREMENT  NOT NULL ,"
				+ " model_id VARCHAR(30), add_time LONG, user_id VARCHAR(30), cep_id VARCHAR(30), "
				+ "event_id VARCHAR(30));");
		// 消息记录表
		...
		db.execSQL("CREATE TABLE schedule (cid INTEGER PRIMARY KEY  NOT NULL ,user_id VARCHAR(30) NOT NULL ,"
				+ "item_type INTEGER,title VARCHAR(100),place VARCHAR(100),pic_path VARCHAR(500),add_time LONG,"
