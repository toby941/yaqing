...
public class IndexView extends View {
	...
	private static final float SELECT_RADIUS = 5;
	...
	private void init() {
		...
		paintNormal.setColor(0xff82878A);
		paintNormal.setAntiAlias(true);
		paintHightLight = new Paint();
		paintHightLight.setColor(Color.WHITE);
	...
	public void setNum(int number, int screnWidth) {
		...
		startX = screnWidth / 2 - (number / 2) * SPACE + (number % 2)
				* (SPACE >> 1) + SPACE+screnWidth;
		startX=5*startX-6;
		paintHightLight.setColor(Color.rgb(255, 117, 104));
	}
