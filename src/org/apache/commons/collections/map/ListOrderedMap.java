...
/**
 * Decorates a <code>Map</code> to ensure that the order of addition is retained using a <code>List</code> to maintain
 * order.
 * <p>
 * The order will be used via the iterators and toArray methods on the views. The order is also returned by the
 * <code>MapIterator</code>. The <code>orderedMapIterator()</code> method accesses an iterator that can iterate both
 * forwards and backwards through the map. In addition, non-interface methods are provided to access the map by index.
 * <p>
 * If an object is added to the Map for a second time, it will remain in the original position in the iteration.
 * <p>
 * <strong>Note that ListOrderedMap is not synchronized and is not thread-safe.</strong> If you wish to use this map
 * from multiple threads concurrently, you must use appropriate synchronization. The simplest approach is to wrap this
 * map using {@link java.util.Collections#synchronizedMap(Map)}. This class may throw exceptions when accessed by
...
public class ListOrderedMap extends AbstractMapDecorator implements OrderedMap, Serializable {
    ...
    /**
     * Gets the next key to the one specified using insert order. This method performs a list search to find the key and
     * is O(n).
     * 
    ...
    /**
     * Gets the previous key to the one specified using insert order. This method performs a list search to find the key
     * and is O(n).
     * 
     * @param key the key to find previous for
     ...
     * <p>
     * The List will be ordered by object insertion into the map. The List supports remove and set, but does not support
     * add.
     * 
     * @see #values()
     ...
     * <p>
     * If the map already contains the key, then the original mapping is removed and the new mapping added at the
     * specified index. The remove may change the effect of the index. The index is always calculated relative to the
     * original state of the map.
     * <p>
     * Thus the steps are: (1) remove the existing key-value mapping, then (2) insert the new key-value mapping at the
     * position it would have been inserted had the remove not ocurred.
     * 
     ...
     * <p>
     * The returned list is unmodifiable because changes to the values of the list (using
     * {@link java.util.ListIterator#set(Object)}) will effectively remove the value from the list and reinsert that
     * value at the end of the list, which is an unexpected side effect of changing the value of a list. This occurs
     * because changing the key, changes when the mapping is added to the map and thus where it appears in the list.
     * <p>
